...
public class EngineeringDAO extends JobDAO{
    ...
    public ArrayList<Worker> queryEmployees(String employerName){
        ...
        try{
            PreparedStatement statement = connection.prepareStatement("SELECT * FROM engineeringoccupations join engineeringjobs ON engineeringoccupations.idEngineeringJob = engineeringjobs.idEngineeringJob join workers ON engineeringoccupations.idWorker = workers.idWorker where idEmployer = ?;");
            statement.setInt(1, employerID);
    ...
    public boolean applicationExists(int idWorker, int idJob){
        ...
        try {
            PreparedStatement getApplicationSet = connection.prepareStatement("SELECT * FROM engineeringapplications WHERE idWorker = ? AND idEngineeringJob = ?");
    ...
    public void createApplication(String workerName, Job job){
        ...
        try {
            ...
            PreparedStatement addApplication = connection.prepareStatement("INSERT INTO engineeringapplications (idWorker, idEngineeringJob) VALUES (?, ?)");
    ...
    public void hireWorker(Worker worker, Job job){
        ...
        try{
            // hiring the worker
            PreparedStatement hireWorker = connection.prepareStatement("INSERT INTO engineeringoccupations (idWorker, idEngineeringJob) VALUES (?, ?)");
            ...
            PreparedStatement clearApps = connection.prepareStatement("DELETE FROM engineeringapplications WHERE idEngineeringJob = ?");
            clearApps.setInt(1, idJob);
            clearApps.executeUpdate();

            // clear the worker's applications toward other jobs once hired
            PreparedStatement clearApps2 = connection.prepareStatement("DELETE FROM applications WHERE idWorker = ?");
